...
var ModEngineRuntime = (function(){
    ...
    function drawBridgeFlow(build){
        try{
            var other = Vars.world.tile(build.link);
            if(other == null || other.build == null) return;
            var item = null;
            try{ if(build.items != null && build.items.total() > 0) item = build.items.first(); }catch(eItems){}
            if(item == null){
                try{ item = build.lastItem; }catch(eLast){}
            }
            if(item == null) return;
            var x1 = build.x, y1 = build.y, x2 = other.drawx(), y2 = other.drawy();
            var dst = Math.max(1, Math.sqrt((x2 - x1) * (x2 - x1) + (y2 - y1) * (y2 - y1)));
            var count = Math.max(2, Math.floor(dst / 7));
            var phase = (Time.time / 22) % 1;
            for(var i = 1; i <= count; i++){
                var f = ((i / (count + 1)) + phase) % 1;
                drawItemIcon(item, x1 + (x2 - x1) * f, y1 + (y2 - y1) * f, 5.8, 0.62);
            }
        }catch(e){}
    }
        ...
        try{
            Groups.build.each(cons(function(build){
                try{
                    ...
                    if(build instanceof Conveyor.ConveyorBuild) drawConveyorFlow(build);
                    else if(build instanceof Router.RouterBuild) drawRouterFlow(build);
                    else if(build instanceof Junction.JunctionBuild) drawJunctionFlow(build);
                    else if(build instanceof ItemBridge.ItemBridgeBuild || build instanceof BufferedItemBridge.BufferedItemBridgeBuild) drawBridgeFlow(build);
                }catch(eBuild){}
            }));
        }catch(e){}
    }
